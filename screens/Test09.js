...
import SwipePadGeoFunc01 from "./components/SwipePadGeoFunc01";

export default function Test09({ navigation }) {
  ...
  const gestureSwipeOnChange = Gesture.Pan().onChange((event) => {
    ...
    const boundary60 = relativeToPadCenterX * Math.tan((Math.PI / 180) * 60);
    if (inInnerCircle) {
      handleSwipeColorChange("center");
      setCurrentActionType(null);
    } else {
      // console.log(`relativeToPadCenterX: ${relativeToPadCenterX}`);
      console.log(`boundary45: ${boundary45}, when y: ${relativeToPadCenterY}`);
